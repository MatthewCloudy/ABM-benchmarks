...
function parse_args()
    params = Dict(
        :agents => 1000,    # całkowita liczba agentów
        :ratio  => 0.2,     # procent wilków
        :steps  => 500,
        :dims   => (20, 20)
    )
...
n_steps = 500
times = Float64[]   # czas każdego kroku

for step in 1:n_steps
    t = @elapsed step!(model)
    push!(times, t)
    # println("Krok $step: $t s")
end

total_time = sum(times)

println("\n=== PODSUMOWANIE ===")
println("Całkowity czas (suma kroków): $total_time s")
println("Średni czas kroku: $(mean(times)) s")
println("Mediana czasu kroku: $(median(times)) s")
println("Najszybszy krok: $(minimum(times)) s")
println("Najwolniejszy krok: $(maximum(times)) s")